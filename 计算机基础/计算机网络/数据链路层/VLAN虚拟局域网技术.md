...
## 什么是VLAN
虚拟局域网(`Virtual Local Area Network，VLAN`)是将一个物理的局域网在**逻辑上划分成多个广播域**的技术。通过在交换机上配置VLAN，实现在同一个VLAN内的主机可以相互通信，而不同VLAN间的主机被相互隔离。
...
- Access端口在发送出站数据帧之前，**会判断这个要被转发的数据帧中携带的VLAN ID是否与出站端口的PVID相同**，若相同则去掉VLAN标签进行转发；若不同则丢弃。
